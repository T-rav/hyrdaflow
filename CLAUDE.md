...
| Cross-cutting standards | [`docs/standards/`](docs/standards/) | Repo-wide rules that apply to every HydraFlow-format project. **Read before deciding whether to ship a fix vs ask permission, or how many test layers a feature needs.** Current standards: [`factory_autonomy/`](docs/standards/factory_autonomy/README.md) — when agents should act vs ask; [`testing/`](docs/standards/testing/README.md) — three-layer test pyramid (unit + MockWorld scenario + sandbox e2e); [`branch_protection/`](docs/standards/branch_protection/README.md) — repeatable ruleset configs for the two-tier branch model (ADR-0042). |

"""Crate (milestone) route handlers for the dashboard API."""
...
def create_crate_router(deps: RouterDeps) -> APIRouter:
    """Create an APIRouter with all crate/milestone endpoints."""
    router = APIRouter()
    ...
    async def get_crates() -> JSONResponse:
        ...
        try:
            crates = await deps.pr_manager.list_milestones()
            result = []
            for crate in crates:
    ...
    async def create_crate(body: CrateCreateRequest) -> JSONResponse:
        ...
        try:
            crate = await deps.pr_manager.create_milestone(
    ...
    async def update_crate(crate_number: int, body: CrateUpdateRequest) -> JSONResponse:
        ...
        try:
            crate = await deps.pr_manager.update_milestone(crate_number, **fields)
            return JSONResponse(crate.model_dump())
        except RuntimeError as exc:
    ...
    async def delete_crate(crate_number: int) -> JSONResponse:
        ...
        try:
            await deps.pr_manager.delete_milestone(crate_number)
    ...
    async def add_crate_items(
        ...
        try:
            for issue_number in body.issue_numbers:
                await deps.pr_manager.set_issue_milestone(issue_number, crate_number)
    ...
    async def remove_crate_items(
        ...
        """
        try:
            current_issues = await deps.pr_manager.list_milestone_issues(crate_number)
            current_nums = {i.get("number") for i in current_issues}
            removed = 0
            for issue_number in body.issue_numbers:
                if issue_number in current_nums:
                    await deps.pr_manager.set_issue_milestone(issue_number, None)
                    removed += 1
    ...
    async def get_active_crate() -> JSONResponse:
        ...
        active_number = deps.state.get_active_crate_number()
        result: dict[str, Any] = {
            ...
            "closed_issues": 0,
            "total_issues": 0,
            "auto_crate": deps.config.auto_crate,
        }
        if active_number is not None and orch is not None:
            try:
                crates = await deps.pr_manager.list_milestones(state="all")
    ...
    async def set_active_crate(body: dict[str, Any]) -> JSONResponse:
        ...
        if orch is None:
            ...
            deps.state.set_active_crate_number(crate_number)
        ...
        if crate_number is not None:
            ...
            deps.state.set_active_crate_number(None)
        return JSONResponse({"status": "ok", "crate_number": crate_number})
    ...
    async def advance_crate() -> JSONResponse:
        ...
        try:
            crates = await deps.pr_manager.list_milestones(state="open")
            candidates = sorted(

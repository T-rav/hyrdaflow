"""Control, admin, config, and system worker route handlers for the HydraFlow dashboard."""
...
def _build_system_worker_inference_stats(
    ...
    """Aggregate inference telemetry per worker."""
    ...
    for worker_name, _label, _description in _bg_worker_defs:
        ...
        totals = {"inference_calls": 0, "total_tokens": 0, "pruned_chars_total": 0}
...
def _compute_next_run(last_run: str | None, interval_seconds: int | None) -> str | None:
    """Calculate the next scheduled run time from a last-run ISO timestamp."""
...
def register_control_routes(router: APIRouter, ctx: RouterContext) -> None:
    """Register control, admin, config, and system worker routes on *router*."""
    ...
    async def start_orchestrator() -> JSONResponse:
        ...
        new_orch = HydraFlowOrchestrator(
            ctx.config, event_bus=ctx.event_bus, state=ctx.state
        )
    ...
    async def get_control_status(repo: RepoSlugParam = None) -> JSONResponse:
    ...
    async def patch_config(
        ...
        if repo and ctx.repo_store is not None and applied:
    ...
    async def get_system_workers(repo: RepoSlugParam = None) -> JSONResponse:
        _cfg, _state, _bus, _get_orch = ctx.resolve_runtime(repo)
        orch = _get_orch()

"""Metrics, runs, insights, retrospectives, memories, and troubleshooting route handlers."""
...
def register_metrics_routes(router: APIRouter, ctx: RouterContext) -> None:
    """Register metrics, runs, insights, and related routes on *router*."""

    @router.get("/api/metrics")
    async def get_metrics(repo: RepoSlugParam = None) -> JSONResponse:
    ...
    async def get_github_metrics(repo: RepoSlugParam = None) -> JSONResponse:
    ...
    async def get_metrics_history(repo: RepoSlugParam = None) -> JSONResponse:
        ...
        orch = _get_orch()
        if orch is None:
            snapshots = ctx.load_local_metrics_cache(_cfg)
            return JSONResponse(
    ...
    async def get_memories() -> JSONResponse:
        ...
        items_dir = ctx.config.data_path("memory", "items")
        digest_path = ctx.config.data_path("memory", "digest.md")
        ...
        curated_store = CuratedManifestStore(ctx.config)

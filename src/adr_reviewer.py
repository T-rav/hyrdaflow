...
minority_note: <dissenting opinion if not unanimous, or "none">"""
    ...
    async def _route_pre_validation_failure(
        ...
        """Route a pre-validation failure by writing to adr_decisions.jsonl.

        Deduplicates locally by checking adr_decisions.jsonl for an existing
        entry with the same title before writing a new one.
        """
        title = f"[ADR Pre-validation] ADR-{adr_number:04d}: structural issues"
        if len(title) > 70:
            title = title[:67] + "..."

        # Local dedup: skip if this title already exists in adr_decisions.jsonl
        if self._adr_decision_exists(title):
            logger.debug(
                "ADR-%04d pre-validation decision already recorded — skipping",
                adr_number,
            )
            stats["pre_validation_skipped"] += 1
            return
        ...
        _write_adr_decision(self._config, title, body, "pre_validation")
        logger.info("ADR-%04d pre-validation recorded to JSONL", adr_number)
        stats["auto_triaged"] += 1

    def _adr_decision_exists(self, title: str) -> bool:
        """Return True if an entry with *title* already exists in adr_decisions.jsonl."""
        try:
    ...
    async def _route_result(
        ...
        if result.duplicate_detected:
            ...
            await self._execute_triage_or_hitl(result, reason="rejected", stats=stats)
            stats["rejected"] += 1
            ...
            if auto_accepted:
                stats["accepted"] += 1
                return
            await self._execute_triage_or_hitl(
                result, reason="changes_requested", stats=stats
            )
        else:
            await self._execute_triage_or_hitl(
                result, reason="no_consensus", stats=stats
            )
    ...
    async def _route_to_triage(self, result: ADRCouncilResult, *, reason: str) -> bool:
        ...
        logger.info(
            "ADR-%04d follow-up decision recorded to JSONL (%s)",
            result.adr_number,
            reason,
        )
        return True
    ...
    async def _handle_duplicate(
        ...
        logger.info(
            "ADR-%04d duplicate decision recorded to JSONL",
            result.adr_number,
        )
        stats["auto_triaged"] += 1

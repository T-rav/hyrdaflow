...
_PREAMBLE = (
    ...
    "All Fake adapters under `src/mockworld/fakes/` (classes with "
    "``_is_fake_adapter = True``; nested-record dataclasses are filtered "
    "out), the `*Port` each implements (by name match), and the scenarios "
    "that wire them. Per ADR-0022 (MockWorld) and ADR-0047 "

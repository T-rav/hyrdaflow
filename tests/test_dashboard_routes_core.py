...
# ---------------------------------------------------------------------------
# GET /api/prs — merged overlay
# ---------------------------------------------------------------------------


class TestGetPrsEndpoint:
    """Tests for the /api/prs merged overlay logic."""

    @pytest.mark.asyncio
    async def test_get_prs_overlays_merged_flag_from_issue_store(
        self, config, event_bus, state, tmp_path
    ) -> None:
        """Items whose issue number is in IssueStore._merged_numbers must have merged=True."""
        import json

        from models import PRListItem

        mock_orch = MagicMock()
        mock_orch.issue_store.get_merged_numbers.return_value = frozenset({42})

        router, pr_mgr = make_dashboard_router(
            config, event_bus, state, tmp_path, get_orch=lambda: mock_orch
        )
        pr_mgr.list_open_prs = AsyncMock(
            return_value=[
                PRListItem(pr=101, issue=42, title="Fix widget"),
                PRListItem(pr=102, issue=99, title="Other PR"),
            ]
        )
        ...
        merged_flags = {item["issue"]: item["merged"] for item in data}
        assert merged_flags[42] is True
        assert merged_flags[99] is False

    @pytest.mark.asyncio
    async def test_get_prs_no_orchestrator_returns_items_without_overlay(
        self, config, event_bus, state, tmp_path
    ) -> None:
        """When orch is None, items are returned as-is (merged=False by default)."""
        import json
        ...
        router, pr_mgr = make_dashboard_router(config, event_bus, state, tmp_path)
        pr_mgr.list_open_prs = AsyncMock(
            return_value=[PRListItem(pr=101, issue=42, title="Fix widget")]
        )

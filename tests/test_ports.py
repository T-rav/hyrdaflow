...
class TestStateBackendPortConformance:
    """DoltBackend must satisfy the StateBackendPort protocol."""

    def test_dolt_backend_satisfies_state_backend_port(self) -> None:
        """DoltBackend is a structural subtype of StateBackendPort."""
        from dolt_backend import DoltBackend
        from ports import StateBackendPort

        # Use __new__ to avoid __init__ (which requires dolt CLI)
        instance = DoltBackend.__new__(DoltBackend)
        assert isinstance(instance, StateBackendPort), (
            "DoltBackend no longer satisfies the StateBackendPort protocol. "
            "Check that all methods declared in StateBackendPort exist on DoltBackend."
        )

    def test_mock_satisfies_state_backend_port(self) -> None:
        """A MagicMock spec'd to StateBackendPort is accepted."""
        from ports import StateBackendPort

        mock: StateBackendPort = MagicMock(spec=StateBackendPort)  # type: ignore[assignment]
        assert isinstance(mock, StateBackendPort)


class TestStateBackendPortMethods:
    """All methods declared in StateBackendPort exist on DoltBackend."""
    ...
    @pytest.mark.parametrize("method", _REQUIRED_METHODS)
    def test_method_exists_on_dolt_backend(self, method: str) -> None:
        from dolt_backend import DoltBackend

        assert hasattr(DoltBackend, method), (
            f"DoltBackend is missing '{method}' which is declared in StateBackendPort"
        )


class TestStateBackendPortSignatures:
    """StateBackendPort method signatures must match DoltBackend's implementations."""
    ...
    @pytest.mark.parametrize("method", _SIGNED_METHODS)
    def test_signature_matches_dolt_backend(self, method: str) -> None:
        from dolt_backend import DoltBackend
        from ports import StateBackendPort

        result = _assert_param_names_match(StateBackendPort, DoltBackend, method)
        assert result is None

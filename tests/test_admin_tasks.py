...
class TestRunCompact:
    """Tests for the async run_compact helper."""
    ...
    def _setup_config(
        ...
        monkeypatch.setattr(
            HydraFlowConfig, "memory_dir", property(lambda self: mem_dir)
        )
        return config

    @pytest.mark.asyncio
    async def test_compact_evicts_auto_evict_items(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        assert result.success is True
        remaining = json.loads(scores_file.read_text())
    ...
    async def test_compact_no_eviction_candidates(
        ...
        assert result.success is True
        assert any("no items" in line.lower() for line in result.log)

    @pytest.mark.asyncio
    async def test_compact_no_scores_file(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        config = self._setup_config(tmp_path, monkeypatch)
    ...
    @pytest.mark.asyncio
    async def test_compact_reports_needs_curation_as_warnings(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        config = self._setup_config(tmp_path, monkeypatch)
        mem_dir = tmp_path / "memory"
        ...
        assert result.success is True
        assert any("curation" in w.lower() for w in result.warnings)

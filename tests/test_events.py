...
_EVENT_STRING_CASES: list[tuple[EventType, str]] = [
    ...
    (EventType.REPORT_UPDATE, "report_update"),
...
# ---------------------------------------------------------------------------
# EventLog.maybe_rotate
# ---------------------------------------------------------------------------


class TestEventLogMaybeRotate:
    """Tests for the background-callable maybe_rotate() method."""

    @pytest.mark.asyncio
    async def test_returns_false_when_file_missing(self, tmp_path: Path) -> None:
        log_path = tmp_path / "missing.jsonl"
        event_log = EventLog(log_path)
        assert await event_log.maybe_rotate() is False

    @pytest.mark.asyncio
    async def test_returns_false_when_under_threshold(self, tmp_path: Path) -> None:
        log_path = tmp_path / "events.jsonl"
        log_path.write_text("small\n")
        event_log = EventLog(log_path)
        assert await event_log.maybe_rotate(max_size_bytes=1024) is False

    @pytest.mark.asyncio
    async def test_returns_true_and_rotates_when_over_threshold(
        self, tmp_path: Path
    ) -> None:
        log_path = tmp_path / "events.jsonl"
        event = HydraFlowEvent(type=EventType.PHASE_CHANGE, data={"x": 1})
        log_path.write_text((event.model_dump_json() + "\n") * 100)
        event_log = EventLog(log_path)
        result = await event_log.maybe_rotate(max_size_bytes=10, max_age_days=365)
        assert result is True

    @pytest.mark.asyncio
    async def test_stat_oserror_returns_false(self, tmp_path: Path) -> None:
        log_path = tmp_path / "events.jsonl"
        log_path.write_text("data\n")
        event_log = EventLog(log_path)
        # Remove the file after exists() check would pass but before stat()

...
class TestSaveConfigFileAtomicAndLogging:
    ...
    def test_logs_warning_on_corrupt_json(
        ...
        """Corrupt JSON in existing file should log a warning and start fresh."""
        config_path = tmp_path / "config.json"
        config_path.write_text("not valid json {{{")

        with caplog.at_level(logging.WARNING, logger="hydraflow.config"):
            save_config_file(config_path, {"max_reviewers": 4})

        assert any("Failed to read config file" in r.message for r in caplog.records)
        loaded = load_config_file(config_path)
        assert loaded["max_reviewers"] == 4

    def test_logs_warning_on_non_dict_json(
        self, tmp_path: Path, caplog: pytest.LogCaptureFixture
    ) -> None:
        """Non-dict JSON in existing file should log a warning and start fresh."""
        config_path = tmp_path / "config.json"
        config_path.write_text(json.dumps([1, 2, 3]))

        with caplog.at_level(logging.WARNING, logger="hydraflow.config"):
            save_config_file(config_path, {"max_workers": 2})

        assert any("non-dict JSON" in r.message for r in caplog.records)
        loaded = load_config_file(config_path)
        assert loaded["max_workers"] == 2

    def test_logs_debug_on_missing_file(
        self, tmp_path: Path, caplog: pytest.LogCaptureFixture
    ) -> None:
        """Missing file should log a debug message and create the file."""
        config_path = tmp_path / "new" / "config.json"

        with caplog.at_level(logging.DEBUG, logger="hydraflow.config"):
            save_config_file(config_path, {"max_workers": 2})
